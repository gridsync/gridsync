...
@ensureDeferred
async def test_magic_folder_creates_conflict_files(
    tmp_path, alice_magic_folder, bob_magic_folder
):
    folder_name = randstr()
    ...
    alice_filepath = alice_path / "file"
    bob_filepath = bob_path / "file"

    alice_filepath.write_text(randstr(128))
    bob_filepath.write_text(randstr(128))

    conflict_file = bob_path / "file.conflict-Alice"
...
async def test_magic_folder_conflict_emits_upload_started_signal(
    ...
    with qtbot.wait_signal(
        bob_magic_folder.events.upload_started, timeout=60000
    ) as blocker:
        alice_filepath = alice_path / "file"
        bob_filepath = bob_path / "file"
        alice_filepath.write_text(randstr(128))
...
@ensureDeferred
async def test_magic_folder_conflict_emits_download_started_signal(
    tmp_path, alice_magic_folder, bob_magic_folder, qtbot
):
    folder_name = randstr()
    ...
    with qtbot.wait_signal(
        bob_magic_folder.events.download_started, timeout=60000
    ) as blocker:
        alice_filepath = alice_path / "file"
        bob_filepath = bob_path / "file"
        alice_filepath.write_text(randstr(128))
...
@ensureDeferred
async def test_magic_folder_conflict_emits_error_occurred_signal(
    tmp_path, alice_magic_folder, bob_magic_folder, qtbot
):
    folder_name = randstr()
    ...
    with qtbot.wait_signal(
        bob_magic_folder.events.error_occurred, timeout=60000
    ) as blocker:
        alice_filepath = alice_path / "file"
        bob_filepath = bob_path / "file"
        alice_filepath.write_text(randstr(128))
