...
git clone --branch master https://github.com/pyenv/pyenv.git ~/.pyenv || git --git-dir=$HOME/.pyenv/.git pull --force --ff origin master || git --git-dir=$HOME/.pyenv/.git reset --hard origin/master || git --git-dir=$HOME/.pyenv/.git merge origin/master
